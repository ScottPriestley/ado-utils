...
| --- | --- |
| `QueryUrl` | Saved query URL to inspect. |
...
`QueryUrl` must be a `dev.azure.com` saved-query URL containing a GUID query ID. The manifest is CSV generated by discovery mode; do not hand-build or reuse it for a different query or result set.

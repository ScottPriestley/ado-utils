...
function Get-TargetStateDirectory {
    <#
        ...
        -StateRoot overrides the computed default entirely, the same way -RunRoot
        already does for New-RunDirectory. Needed on ephemeral CI agents, where
        MyDocuments doesn't persist between runs: a caller can point this at a
        path a cache/restore step manages instead.
    #>
    param([Parameter(Mandatory)]$Target)
    $path = if ([string]::IsNullOrWhiteSpace($StateRoot)) {
        ...
        Join-Path (Join-Path $base $targetPart) 'state'
    } else {
